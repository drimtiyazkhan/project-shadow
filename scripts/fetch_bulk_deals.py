...
def parse_csv(text: str) -> pd.DataFrame:
    ...
    if missing:
        log.error("Could not find a matching column for: %s", missing)
        log.error("Columns actually present in the fetched file: %s", list(raw.columns))
        raise ValueError(f"Unexpected CSV format from NSE — missing fields: {missing}")
